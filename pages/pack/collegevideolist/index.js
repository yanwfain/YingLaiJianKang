// pages/pack/collegevideolist/index.js
var app = getApp()
Page({
    ...
    app.wxRequest('get', url, data, (res) => {
      ...
      if (res.success) {
        ...
        if(res.data.videoList.length>0){
          this.setData({
            videoid:res.data.videoList[0].id,
            videoUrl:res.data.videoList[0].video
          })
        }
        that.getMore()
      } else {
  ...
  bindvideo:function(e){
    this.setData({
      ...
      videoUrl:e.currentTarget.dataset.video,
  ...
  onShareAppMessage: function () {
    ...
    var shareObj = {
      ...
      path: '/pages/pack/collegevideolist/index?id=' + that.data.list_index.id + 'user_uuid=' + app.globalData.user_id,        // 默认是当前页面，必须是以‘/’开头的完整路径
    ...
    if (e.from == 'button') {
      ...
      app.wxRequest('POST', url, data, (res) => {
        ...
        wx.hideLoading()
        if (res.code == 1) {
          shareObj.path = '/pages/pack/collegevideolist/index?btn_name=' + eData.name + '&id=' + that.data.id + '&user_uuid=' + app.globalData.user_id +'&forward_id =' + res.data.id;
        } else {
          wx.showToast({
            title: '提交失败',
            icon: 'none'
