...
Page({
  ...
  onLoad: function (options) {
    ...
    var that = this
    var url = app.globalData.url + '/camper/getReportByUser';
    var data = {
    ...
    app.wxRequest('POST', url, data, (res) => {
      ...
      if (res.success&&res.data) {
        that.setData({
          ...
          date : res.data.inspect_date ? res.data.inspect_date : '',
  ...
  baocunsx:function(e){
    var that = this
    var url = app.globalData.url + '/joinCamp/saveReport';
    var data = {
      user_id: app.globalData.user_id,
      inspect_date: that.data.date ? that.data.date : '',
      electrocardiogram: JSON.stringify(that.data.imgbox) ,
      neck: JSON.stringify(that.data.imgboxs) ,
      abdomen: JSON.stringify(that.data.imgboxs1) ,
      fundus: JSON.stringify(that.data.imgboxs12) ,
      other: JSON.stringify(that.data.imgboxs13) ,
      step: 2,
      piece: 4,
      aim: 2,
  ...
  xiapageFn:function(e){
    if(!this.data.date||this.data.imgbox.length==0||this.data.imgboxs.length==0||this.data.imgboxs1.length==0||this.data.imgboxs12.length==0||this.data.imgboxs13.length==0){
      wx.showToast({
        title: '请完善信息后提交',
        icon:'none'
      })
