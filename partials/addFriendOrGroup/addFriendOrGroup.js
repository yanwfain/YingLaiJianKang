...
let pageConfig = {
  ...
  searchFriendResult(err, user) {
    ...
    if (user) {
      if (user.account == this.data.userInfo.account) { //自己
        ...
        if (isFriend) {//好友
          wx.navigateTo({
            url: '/pages/personcard/personcard?account=' + user.account,
          })
        } else {//陌生人
          ...
          wx.navigateTo({
            url: '/pages/strangercard/strangercard?account=' + user.account,
          })
        }
      }
    } else {
  ...
  searchGroupResult(err, group) {
    ...
    if (group && group.type === 'advanced') {
      ...
      wx.navigateTo({
        url: '/pages/advancedGroupCard/advancedGroupCard?teamId=' + group.teamId
      })
    } else {
