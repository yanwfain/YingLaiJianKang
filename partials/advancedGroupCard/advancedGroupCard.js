...
let pageConfig = {
  ...
  addPerson () {
    wx.navigateTo({
      url: '/pages/../partials/chooseContact/chooseContact?type=advanced&action=add&teamId=' + this.data.teamId + '&limit=' + (this.data.isManager || this.data.isOwner || this.data.inviteMode === 'all')
    })
  },
  /**
   * 打开修改弹框
  ...
  lookAnnouncement() {
    wx.navigateTo({
      url: '/pages/../partials/announcement/announcement?limit=' + (!this.data.currentGroup.isCurrentNotIn && (this.data.isManager || this.data.isOwner || this.data.updateTeamMode === 'all')) + '&teamId=' + this.data.teamId
    })
  },
  /**
   * 查看群成员
  ...
  lookMembers(e) {
    ...
    if (add) { // 添加群成员
      ...
      wx.navigateTo({
        url: '/pages/../partials/advancedGroupMember/advancedGroupMember?teamId=' + this.data.teamId
      })
    }
  },
  /**
